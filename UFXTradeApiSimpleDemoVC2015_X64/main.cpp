...
2.拷贝XTA_W32\Api\UFX_VX.X.X.XXX目录（X.X.X.XXX为版本号）下的所有dll文件到可执行文件目录，
...
{
	printf("  Tick, %s, HighestPrice=%g, LowestPrice=%g, BidPrice0=%g, BidVolume0=%lld, AskPrice0=%g, AskVolume0=%lld, LastPrice=%g, TotalVolume=%lld, TradingDay=%s, TradingTime=%s\n",
...
{
	printf("  ProductType=%d, Ref=%s, ID=%s, InstID=%s, Price=%g, Volume=%lld, NoTradedVolume=%lld, Direction=%d, OpenCloseType=%d, PriceCond=%d, TimeCond=%d, VolumeCond=%d, Status=%d, Msg=%s, %s\n",
		order.ProductType,
...
{
	printf("  InstID=%s, Direction=%d, PosToday=%lld, PosYesterday=%lld\n",
